...
class uploadClass
{
	...
	{
		$fileRequested = str_replace($_SESSION['boot']->getAppPublic()."upload/uploads/","",$_SERVER['REQUEST_URI']);

		//lets check if this file exists...
		$exists = file_exists("../uploads/" . $fileRequested);

		if ($exists)
		{
			$accessible = true;

			if (isset($_SESSION) && isset($_SESSION['user_level']) && $_SESSION['user_level'] > 2)
			{
				$res = db_fetch("","","","","Technical_Proposal_UPLOAD = '".$fileRequested."' or Financial_Proposal_UPLOAD = '".$fileRequested."'","","applications",false, false, null);
				...
				if (sizeOf($res) > 0)
				{
					...
					foreach ($res as $rs)
					{
						...
						//check if process deadline has passed
						require_once "../controller/process.php";
						$classProcess = new processClass();
						$classProcess->clazz = "ProcessClass";
						$classProcess->id = $processId;
						$process = db_fetch("","","","","id = ".$processId,"","process",false, false, null);
						if (sizeOf($process) > 0)
						{
							$resp = $classProcess->processChecklist($process[0], true, false);
							$milestoneDates = $resp['milestoneDates'];

							//Receipt is the ITT Deadline Date
							$deadline = $milestoneDates['Receipt'];

							if (strtotime(date('d-m-Y')) > $deadline)
							{
								$passed = true;
							}
						}
				...
				/*
				$res = db_fetch("","","","","attachment = '".$fileRequested."'","","applications_attachments",false, false, null);
				if (sizeOf($res) > 0)
				{
					foreach ($res as $rs)
					{
						$applicationId = $rs['applications_ID'];
						$appl = db_fetch("","","","","id = ".$applicationId,"","applications",false, false, null);

						$processId = 0;
						if (sizeOf($appl) > 0)
						{
							$processId = $appl[0]['process_ID'];
						}
						...
						if ($processId != 0)
						{
							...
							if (sizeOf($process) > 0)
							{
								...
								if (strtotime(date('d-m-Y')) < $deadline)
								{
									logger(__FILE__,__LINE__,__CLASS__,__METHOD__,"is before");
									$passed = false;
								}
							}
							//if not passed, $passed = false;
...
*/
			...
			//lets ensure only Procurement and ITT Panel can access the documents...
			if (isset($_SESSION) && isset($_SESSION['user_level']) && $_SESSION['user_level'] > 3)
			{
				$block = true;

				$processId = "";

				//lets check if the logged in is an ITT panel member...
				$res = db_fetch("","","","","Technical_Proposal_UPLOAD = '".$fileRequested."' or Financial_Proposal_UPLOAD = '".$fileRequested."'","","applications",false, false, null);
				...
				if ($processId != "")
				{
					...
					if ($process[0]['ITT_Shortlist_Panel_ID'] != null)
					{
						$ITTPanel = $process[0]['ITT_Shortlist_Panel_ID'];

						$panel = db_fetch("","","","","process_panel_ID = ".$ITTPanel,"","panel", false, false, null);

						foreach ($panel as $pan)
						{
							if ($pan['user_ID'] == $_SESSION['user_logged'])
							{
								$block = false;
							}
						}
			...
			$accessible = true;
			$block = false;
			//allow download if accessible
			if ($accessible && $block == false)
			{
				//serve the file
				$file = "../uploads/" . $fileRequested;
			...
			{
				$view[0]->error = "File access denied. Either you do not have access to the file or it cannot be viewed right now.";
				return renderViewForward('index','index',$view,null);
			}
		}
		else
		{
			$view[0]->error = "File cannot be found!";
			return renderViewForward('index','index',$view,null);
		}
	}
	...
	public function upload()
	{
		$options = array(
		    'delete_type' => 'POST',
		    'db_host' => $_SESSION['boot']->host,
		    'db_user' => $_SESSION['boot']->user,
		    'db_pass' => $_SESSION['boot']->password,
		    'db_name' => $_SESSION['boot']->database,
		    'db_table' => 'files'
		);


		$uploader = new CustomUploadHandler($options);
